...
[CmdletBinding()]
param(
    # CDM only settings (optional, CDMState overides when not explicitly set)
    [ValidateSet('Enabled','Disabled')][string]$WelcomeContent = $(if ($env:welcomeContent) { $env:welcomeContent } else { $null }),
    [ValidateSet('Enabled','Disabled')][string]$LockScreenContent = $(if ($env:lockScreenContent) { $env:lockScreenContent } else { $null }),
    [ValidateSet('Enabled','Disabled')][string]$StartContent = $(if ($env:startContent) { $env:startContent } else { $null }),
    [ValidateSet('Enabled','Disabled')][string]$SettingsContent = $(if ($env:settingsContent) { $env:settingsContent } else { $null }),
    [ValidateSet('Enabled','Disabled')][string]$InstallContent = $(if ($env:installContent) { $env:installContent } else { $null }),
    [ValidateSet('Enabled','Disabled')][string]$PreInstalledAppsContent = $(if ($env:preInstalledAppsContent) { $env:preInstalledAppsContent } else { $null }),
    [ValidateSet('Enabled','Disabled')][string]$SuggestionsContent = $(if ($env:suggestionsContent) { $env:suggestionsContent } else { $null }),
    [ValidateSet('Enabled','Disabled')][string]$FeatureManagementContent = $(if ($env:featureManagementContent) { $env:featureManagementContent } else { $null }),
    [ValidateSet('Enabled','Disabled')][string]$AppsContent = $(if ($env:appsContent) { $env:appsContent } else { $null }),
    [ValidateSet('Enabled','Disabled')][string]$PeopleContent = $(if ($env:peopleContent) { $env:peopleContent } else { $null }),
    [ValidateSet('Enabled','Disabled')][string]$TimelineContent = $(if ($env:timelineContent) { $env:timelineContent } else { $null }),
    [ValidateSet('Enabled','Disabled')][string]$SpotlightContent = $(if ($env:spotlightContent) { $env:spotlightContent } else { $null }),
    
    # Mixed CDM + other hives. Note: will only set the CDM options (CDMState overides when not explicitly set)
    [ValidateSet('Enabled','Disabled')][string]$SyncProvidersContent = $(if ($env:syncProvidersContent) { $env:syncProvidersContent } else { $null }),
    [ValidateSet('Enabled','Disabled')][string]$WindowsInkContent = $(if ($env:windowsInkContent) { $env:windowsInkContent } else { $null }),
    
    # Non CDM settings
    [ValidateSet('Enabled','Disabled')][string]$UserProfileEngagementContent = $(if ($env:userProfileEngagementContent) { $env:userProfileEngagementContent } else { $null }), # Removed quietly by microsoft for 23H2 and up... This will result in a single error if explicitley set on new versions.
    [ValidateSet('Enabled','Disabled')][string]$BackgroundAccessContent = $(if ($env:backgroundAccessContent) { $env:backgroundAccessContent } else { $null }),
    [ValidateSet('Enabled','Disabled')][string]$SyncProvidersExtrasContent = $(if ($env:syncProvidersExtrasContent) { $env:syncProvidersExtrasContent } else { $null }),
    [ValidateSet('Enabled','Disabled')][string]$WindowsInkExtrasContent = $(if ($env:windowsInkExtrasContent) { $env:windowsInkExtrasContent } else { $null }),
    
    # General options
    [switch]$IncludeDefaultHive = $(if ($env:includeDefaultHive) { [Convert]::ToBoolean($env:includeDefaultHive) } else { $false }),
    [ValidateSet('Allow','Block')][string]$CDMState = $(if ($env:cdmState) { $env:cdmState } else { $null }),
...
begin {
    ...
    function Write-Log {
        ...
        if ($SaveLogToDevice) {
            ...
            $logFile = Join-Path $logDir "CDM.log"
