...
[CmdletBinding()]
param(
    [ValidateSet("Eastern","Central","Mountain","Pacific")][string]$TimeZone = $(if ($env:timeZone) { $env:timeZone } else { "Central" })
)
...
begin {
    ...
    Write-Output "RMM timeZone env var: '$TimeZone'"
}
...
process {
    ...
    if ($tzMap.ContainsKey($TimeZone)) {
        ...
        Write-Output "Selected time zone shorthand '$TimeZone' -> '$fullTz'"
    }
    else {
    ...
    try {
        tzutil /s "$fullTz"
        Write-Output "Time zone successfully set to '$fullTz'."
    }
    catch {
...
# =========================================
    ...
    Write-Output "Time zone configuration script completed."
}