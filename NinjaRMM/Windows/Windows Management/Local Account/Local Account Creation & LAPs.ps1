...
[CmdletBinding()]
param(
    [Parameter()][String]$AdminUsername                 = $(if ($env:adminUsername)                      { $env:adminUsername }                                            else { "Admin" }),
    [Parameter()][String]$PasswordSecureFieldName       = $(if ($env:passwordSecureFieldName)            { $env:passwordSecureFieldName }                                  else { "LocalAdminPassword" }),
    [Parameter()][Switch]$OverridePassword              = $(if ($env:overridePasswordForExistingAccount) { [Convert]::ToBoolean($env:overridePasswordForExistingAccount) } else { $false }),
    [Parameter()][Switch]$PasswordNeverExpires          = $(if ($env:passwordNeverExpires)               { [Convert]::ToBoolean($env:passwordNeverExpires) }               else { $false }),
    [Parameter()][Switch]$AddToAdministrators           = $(if ($env:addToAdministrators)                { [Convert]::ToBoolean($env:addToAdministrators) }                else { $false })
)
