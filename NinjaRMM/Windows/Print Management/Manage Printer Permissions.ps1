...
[CmdletBinding()]
param(
    [string]$PrinterName = $(if ($env:printerName) { $env:printerName } else { "Not Set!" }),
    [string]$PrinterPrincipal = $(if ($env:printerPrincipal) { $env:printerPrincipal } else { "Not Set!" }),

    [ValidateSet("Allow","Deny")][string]$PrintPermission = $(if ($env:managePrintPermission) { $env:managePrintPermission } else { "Not Set!" }),
    [ValidateSet("Allow","Deny")][string]$ManagePrinterPermission = $(if ($env:managePrinterPermission) { $env:managePrinterPermission } else { "Not Set!" }),
    [ValidateSet("Allow","Deny")][string]$ManageDocumentsPermission $(if ($env:manageDocumentsPermission) { $env:manageDocumentsPermission } else { "Not Set!" }),
    
    [Switch]$RemovePrincipal = $(if ($env:removePrincipal) { [Convert]::ToBoolean($env:removePrincipal) } else { $false }),
