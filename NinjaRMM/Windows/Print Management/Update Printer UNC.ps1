...
[CmdletBinding()]
param(
    [string]$OldUNCPath = $(if ($env:oldUncPath) { $env:oldUncPath } else { "Not Set!" }),
    [string]$NewUNCPath = $(if ($env:newUncPath) { $env:newUncPath } else { "Not Set!" }),
    
    [Switch]$CopyPreferences = $(if ($env:copyPrinterPreferences) { [Convert]::ToBoolean($env:copyPrinterPreferences) } else { $false })
)
