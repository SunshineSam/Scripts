...
[CmdletBinding()]
param(
    [Switch]$SystemUninstall           = $(if ($env:systemUninstall)        { [Convert]::ToBoolean($env:systemUninstall) }        else { $false }),
    [Switch]$PreInstalledUninstall     = $(if ($env:preinstalledRemoval)    { [Convert]::ToBoolean($env:preinstalledRemoval) }    else { $false }),
    [Switch]$UserProfileUninstall      = $(if ($env:userProfileUninstall)   { [Convert]::ToBoolean($env:userProfileUninstall) }   else { $false }),
    [Switch]$ApplyGPO                  = $(if ($env:applyGPO)               { [Convert]::ToBoolean($env:applyGPO) }               else { $false }),
    [Switch]$RemoveFromExplorerSidebar = $(if ($env:removeFromExplorerSidebar) { [Convert]::ToBoolean($env:removeFromExplorerSidebar) } else { $false }),
    [Switch]$RemoveScheduledRun        = $(if ($env:removeScheduledRun)     { [Convert]::ToBoolean($env:removeScheduledRun) }     else { $false }),
    [Switch]$RemoveRunHook             = $(if ($env:removeRunHook)          { [Convert]::ToBoolean($env:removeRunHook) }          else { $false }),
    [Switch]$RestartExplorer           = $(if ($env:restartExplorer)        { [Convert]::ToBoolean($env:restartExplorer) }        else { $false })
)
