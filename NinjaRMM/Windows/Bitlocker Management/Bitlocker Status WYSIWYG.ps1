...
<#
.SYNOPSIS
    Report BitLocker status for all fixed drives and generate HTML status cards for NinjaRMM.

.DESCRIPTION
    This script reports the BitLocker status for all fixed drives on the system, generating HTML status cards
    and collecting recovery keys for display in NinjaRMM custom fields. It does not perform any management actions
    such as enabling, suspending, or disabling BitLocker. Recovery keys for all fixed drives are stored in a single
    secure custom field in a structured text format (e.g., "Drive: C: - ID: {GUID} | Key: {Key}; Drive: D: - ID: {GUID} | Key: {Key};").
...
param(
    ...
    [string]$UpdateRecoveryKeys = $(if ($env:updateRecoveryKeys) { [Convert]::ToBoolean($env:updateRecoveryKeys) } else { $true }),
