...
.PARAMETER AutoReboot
    Switch to reboot after Enablement. (default false)
...
param(
    ...
    [switch]$AutoUnlockNonOSDrives = $(if ($env:autoUnlockNonOsDrives) { [Convert]::ToBoolean($env:autoUnlockNonOsDrives) } else { $true }), # Static
    ...
    [int]$SuspensionRebootCount = $(if ($env:bitlockerSuspensionRebootCount) { [int]$env:bitlockerSuspensionRebootCount } else { 1 }), # Static
    [int]$RebootDelay = 300, # Static
    ...
    # Variable to ensure both TPM and Recovery Key protectors are always present (when true). Subsequently prevents recovery key prompt on every boot.
    [switch]$PreventKeyPromptOnEveryBoot = $(if ($env:preventKeyPromptOnEveryBoot) { [Convert]::ToBoolean($env:preventKeyPromptOnEveryBoot) } else { $true }), # Static
    
    # Registry path for storing unretreivable states
    [string]$BitLockerStateStoragePath = $(if ($env:bitLockerStateStoragePath) { $env:bitLockerStateStoragePath } else { "HKLM:\SOFTWARE\BitLockerManagement" }), # Static
    # Storing UsedSpaceOnly setting
    [string]$UsedSpaceOnlyStateValueName = $(if ($env:usedSpaceOnlyStateValueName) { $env:usedSpaceOnlyStateValueName } else { "UsedSpaceOnly" }), # Static
    # Storing InitialSuspensionCount setting
    [string]$InitialSuspensionCountValueName = $(if ($env:suspensionCountValueName) { $env:suspensionCountValueName } else { "InitialSuspensionCount" }) # Static
)
...
begin {
    ...
    if ($PreventKeyPromptOnEveryBoot) {
        ...
        if ($BitLockerProtection -in @("Enable", "Suspend")) {
            ...
            # If disabling BitLocker with PreventKeyPromptOnEveryBoot, include all BitLocker-enabled volumes
            if ($BitLockerProtection -eq 'Disable') {
                $bitLockerVolumes = Get-BitLockerVolume | Where-Object { $_.KeyProtector } | Select-Object -ExpandProperty MountPoint
                $drives = ($drives + $bitLockerVolumes) | Select-Object -Unique
                Write-Host "[INFO] Automatically including all BitLocker-enabled volumes for disabling due to PreventKeyPromptOnEveryBoot being enabled."
            }
        }
        else {
